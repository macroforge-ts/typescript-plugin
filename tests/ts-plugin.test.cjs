...
`;
    ...
    assert(
        result.includes('export interface Config'),
        'should contain original interface'
    );
    assert(result.includes('export enum Status'), 'should contain original enum');
    assert(
        result.includes('export type Dimensions'),
        'should contain original type'
    );
    ...
    assert(
        !result.includes('defaultValue()'),
        'should not generate defaultValue method'
    );
    ...
    assert(
        !macroDiag,
        'should not have any macro diagnostics for non-macro file'
    );
});
...
test('skips expansion for real-world JSDoc file with @derive in comments (fixture)', (t) => {
    ...
    assert(
        result.includes('export { createFormState }'),
        'should contain original exports'
    );
    assert(
        result.includes('export { gigaformEnhance }'),
        'should contain original exports'
    );
    assert(
    ...
    assert(
        !result.includes('static defaultValue()'),
        'should not generate defaultValue method'
    );
    assert(!result.includes('toJSON()'), 'should not generate toJSON method');
    assert(!result.includes('fromJSON('), 'should not generate fromJSON method');
    ...
    assert(
        !macroDiag,
        'should not have any macro diagnostics for file with @derive only in comments'
    );
});
