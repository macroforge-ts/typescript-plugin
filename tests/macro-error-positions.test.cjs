...
class User {
  id: string;
    ...
    const diagnostics = env.pluginService.getSemanticDiagnostics(
        '/virtual/test.ts'
    );
...
class User {
  ...
  broken: string = ;
    ...
    assert(
        typeof macroDiag.messageText === 'string' &&
            macroDiag.messageText.length > 0,
  ...
  id: string;
    ...
    for (const diag of diagnostics) {
        if (diag.start !== undefined) {
            const isMacroDiag = diag.source === 'macroforge' ||
                diag.source === 'macroforge-generated';
            assert(
...
a.toString();`;
    ...
    const expandedLength = expandedSnapshot ? expandedSnapshot.getLength() : sourceLength;
