...
function loadFixture(fixtureName) {
    ...
    const original = fs.readFileSync(
        path.join(fixtureDir, 'original.test.ts'),
        'utf-8'
    );
    const expanded = fs.readFileSync(
        path.join(fixtureDir, 'expanded.test.ts'),
        'utf-8'
    );
...
function findInsertion(original, expanded) {
    ...
    while (
        insertionPoint < minLen &&
        original[insertionPoint] === expanded[insertionPoint]
    ) {
        insertionPoint++;
    }
    ...
    if (afterOriginal !== afterExpanded) {
        ...
        while (
            ...
            original[original.length - 1 - suffixLen] ===
                expanded[expanded.length - 1 - suffixLen]
        ) {
            suffixLen++;
        }
        ...
        return {
            ...
            insertionLength: expandedEndOfChange - originalEndOfChange +
                (originalEndOfChange - insertionPoint)
        };
    }
