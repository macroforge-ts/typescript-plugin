...
function init(modules: { typescript: typeof ts }) {
    ...
    function create(info: ts.server.PluginCreateInfo) {
            ...
            try {
                ...
                if (generatedCodeDiagnostics.length > 0 && result.sourceMapping) {
                    ...
                    for (const diag of generatedCodeDiagnostics) {
                        ...
                        log(
                            `  -> generatedRegions: ${
                                JSON.stringify(result.sourceMapping!.generatedRegions)
                            }`
                        );
                        log(
                            `  -> deriveDecorators: ${
                                JSON.stringify(
                                    deriveDecorators.map((d) => ({
        ...
        const originalGetTypeDefinitionAtPosition = info.languageService
            .getTypeDefinitionAtPosition.bind(
        ...
        const originalGetImplementationAtPosition = info.languageService
            .getImplementationAtPosition.bind(
                info.languageService
            );
