...
class User {
  name: string;
    ...
    for (const macro of macros) {
        ...
        assert.ok(
            hover.displayParts,
            `hover for ${macro} should have displayParts`
        );
        const displayText = hover.displayParts.map((p) => p.text).join('');
        assert.ok(
...
test('no macro hover on regular code', async (t) => {
  ...
  constructor(name: string) {
    ...
    assert.strictEqual(
        hover,
        undefined,
        'should not have macro hover on regular code'
    );
});
...
class User { name: string; }`;
    ...
    const docText = (hover.documentation ?? []).map((d) => d.text).join('');
    assert.ok(
        docText.includes('Derive directive'),
        'should have derive directive description'
    );
    assert.ok(
        docText.includes('Built-in macros'),
        'should mention built-in macros'
    );
});
...
interface Account { id: string; }`;
    ...
    assert.ok(
        docText.includes('External macro'),
        "should indicate it's an external macro"
    );
});
...
class User {
  ...
  name: string;
    ...
    const docText = (hover.documentation ?? []).map((d) => d.text).join('');
    assert.ok(
        docText.includes('@derive') || docText.includes('Debug') ||
            docText.includes('CustomMacro'),
